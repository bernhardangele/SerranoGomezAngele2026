...
exp2 %>% filter(corr == 1) %>% ggplot(aes(x = Condition, y = rt, color = Condition, fill = Condition)) +
  geom_rain(alpha = .5,
  ...
  theme_minimal() +
  labs(x = "Condition", y = "RT (s)", title = "RT by Condition and Stimulus Type") +
...
qs_save(blmm_exp2_acc_nonword, file = here("analysis", "blmm_exp2_acc_nonword.qs"))
