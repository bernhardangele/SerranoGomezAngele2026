...
exp1_all_participants <- exp1_all_csv_content |>
  ...
  select(source, PROLIFIC_PID, date, OS, frameRate, rt, corr, TrialID, StimulusType, Condition, FlankerDuration, Target, Flanker, response_key_condition, counterbalancing_condition)
...
# Raincloud plot for visualization of data
exp1 %>% filter(corr == 1) %>% ggplot(aes(x = Condition, y = rt, color = Condition, fill = Condition)) +
  ...
  facet_wrap(~StimulusType)

write_csv(file = here("analysis", "exp1_data_for_analysis.csv"), x = exp1)
...
# Test model with lmer
m1 <- lmer(rt ~ Condition*StimulusType + (1|subject) + (1|Target), data = exp1 %>% filter(rt > .25 & corr == 1 & rt < 1.8))


blmm_rt_dist <-
  brm(
    data = exp1 |> filter(rt > .25 & rt < 1.8 & corr == 1),
    formula = bf(
...
qs_save(blmm_rt_dist, file = here("analysis", "blmm_exp1_rt_dist.qs"))
...
qs_save(blmm_acc_exp1, file = here("analysis", "blmm_acc_exp1.qs"))
