...
blmm_rt_dist_opaque <-
  brm(
    data = exp1 |> filter(rt > .25 & rt < 1.8 & corr == 1 & Transparency == "Opaque"),
    formula = bf(
...
qs_save(blmm_rt_dist_opaque, file = here("analysis", "blmm_exp1_rt_dist_opaque.qs"))


blmm_acc_exp1_opaque <-
  brm(
    data = exp1 |> filter(rt > .25 & rt < 1.8 & corr != -1 & Transparency == "Opaque"),
...
qs_save(blmm_acc_exp1_opaque, file = here("analysis", "blmm_exp1_acc_opaque.qs"))
